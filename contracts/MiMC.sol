...
/// @title MiMC Hash Library (Feistel-based Sponge Construction)
/// @notice Pure Solidity implementation of MiMC hash, optimized for zkSNARK-friendly hashing
/// @dev Based on the reference implementation: https://gist.github.com/poma/5adb51d49057d0a0edad2cbd12945ac4
library MiMC {
    // Prime field modulus (BN254 / alt_bn128 curve field size)
    uint256 private constant FIELD_SIZE =
        21888242871839275222246405745257275088548364400416034343698204186575808495617;

    /// @dev One round of MiMC Feistel permutation
    /// @param xL Left half of the state
    /// @param xR Right half of the state
    /// @param cPartial Round constant
    /// @return (xL, xR) Permuted state after one round
    function MiMCFeistel(
        uint256 xL,
        uint256 xR,
        uint256 cPartial
    ) private pure returns (uint256, uint256) {
        // Apply round function: f(t) = t^5 mod FIELD_SIZE
    ...
    /// @dev MiMC Sponge hash function using fixed number of rounds and constants
    /// @param xL Left input to the sponge (can be a message block or zero)
    /// @param xR Right input to the sponge (e.g., capacity or zero)
    /// @return (xL, xR) The new sponge state
    function MiMCSponge(
        uint256 xL,
        uint256 xR
    ) private pure returns (uint256, uint256) {
        ...
        // Initial round (same as Feistel without constant)
        t = xL;
        ...
        // MiMC Feistel rounds with constants c-partials
        (xL, xR) = MiMCFeistel(
            xL,
            xR,
        ...
        // Final round (sponge squeeze step)
        t = xL;
        exp = mulmod(t, t, FIELD_SIZE);
        exp = mulmod(exp, exp, FIELD_SIZE);
        exp = mulmod(exp, t, FIELD_SIZE);
    ...
    function hashLeftRight(
        ...
        // Step 1: absorb left into sponge
        uint256 R = left;
        uint256 C = 0;
        (R, C) = MiMCSponge(R, C);

        // Step 2: absorb right into sponge
